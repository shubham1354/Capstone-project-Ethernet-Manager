...
int is_sync_with_system(void)
{
  ...
  char system_value[MAX];

  while (current != NULL) {
    // get system value for IPV4 and compare it with config file
    if (get_system_value(current->interface_name, "IPV4") == 0) {
      snprintf(system_value, MAX, "%s", ip_address);

      if (strcmp(system_value, current->ipv4.ipv4_address) != 0) {
    ...
    else if (get_system_value(current->interface_name, "MAC") == 0) {
      snprintf(system_value, MAX, "%s", mac);

      if (strcmp(system_value, current->mac.mac_address) != 0) {
        // if not sync --> update system values
        if (set_system_value(
              current->interface_name, "MAC", current->mac.mac_address)
            != 0) {
          fprintf(stderr,
                  "Failed to set MAC for %s in synchronisation\n",
                  current->interface_name);
          return -1;
        }
      }
...
int update_config_file(char *file_path)
{
...
struct confignode *get_node(const char *interface_name)
{
  ...
  struct confignode *new_node = malloc(sizeof(struct confignode));
  if (!new_node) {
...
/* function will check for duplicate specifications */
int validate_duplicate(void)
{
  struct confignode *current = NULL;
  struct confignode *compare = NULL;

  for (current = head; current != NULL; current = current->next) {
    for (compare = current->next; compare != NULL; compare = compare->next) {
      // check for --> Duplicate ipv4 address
      if (strcmp(current->ipv4.ipv4_address, compare->ipv4.ipv4_address) == 0
          && strlen(current->ipv4.ipv4_address) > 0) {
        fprintf(stderr,
      ...
      else if (strcmp(current->mac.mac_address, compare->mac.mac_address) == 0
               && strlen(current->mac.mac_address) > 0) {
        fprintf(stderr,
      ...
      // check for --> Duplicate VLAN ID address
      else if (current->vlan.vlan_id == compare->vlan.vlan_id
               && current->vlan.vlan_id != 0) {
        fprintf(stderr, "Duplicate VLAN ID found: %d\n", current->vlan.vlan_id);
        return -1;
      }
    }
  }
...
int validate_vlan_id(int vlan_id)
{
...
/* Function will give the last node address */
struct confignode *access_last_node(void)
{
  // if no nodes present
  if (head == NULL) {
    fprintf(stderr, "No nodes present\n");
    return NULL;
  }

  struct confignode *current = head;
  struct confignode *prev    = NULL;
