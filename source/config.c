...
/* Function will Creat database by reading a config file 
    and call for the validate */
...
/* function to validate the duplicate and will call to functions
   which will validate the specifications */
int validate_configuration(int line_count)
{
  ...
  for (current = head; current != NULL; current = current->next) {
    // check for --> first 7 digits of gateway and network specifications
...
int validate_duplicate()
{
  ...
  while (current != NULL) {
    ...
    for (compare = current->next; compare != NULL; compare = compare->next) {
      ...
      snprintf(buffer1, sizeof(buffer1), "%d", compare->vlan.vlan_id);
      snprintf(buffer2, sizeof(buffer2), "%d", current->vlan.vlan_id);
...
/* function will check first 7 digit of n/w and gateway of Route are same*/
